...
#'
#' @noRd
get_dest_cell_coarse <- function(rw, x_res, y_res, bfr = 75) { # A from-to cell pair and coordinates of departure point; bfr is a search radius in km for nearest cooler cell, if you end on land
  ...
  clumped <- get_clumps(pos_depart, mn, bfr, x_res, y_res)
...
#'
#' @noRd
get_dest_cell_fine <- function(rw, x_res, y_res) { # A to-from cell pair and the buffer around a cell on land that can be searched for a suitable cell

  # Find clumps of ocean; the rule is that you can't jump from one clump to another, because this would mean passing over unbroken land
  pos_depart <- data.frame(
    x = purrr::pluck(rw, 3),
    y = purrr::pluck(rw, 4)
  )
  ...
  clumped <- get_clumps(xy, mn, bfr, x_res, y_res)
