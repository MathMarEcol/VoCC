## ----setup, include = FALSE---------------------------------------------------
  ...
  terra::terraOptions(memfrac = 0.3, tempdir = tempdir())
  ...
  # Helper function to clean up memory between sections
...
# Use sequential processing during vignette build to minimize memory
...
system.time({
  ...
  # Clean up
  rm(coords, missing_points)
  if(exists("marine_cells")) rm(marine_cells)
  if(exists("nearest_indices")) rm(nearest_indices)
  if(exists("nearest_cells")) rm(nearest_cells)
  if(exists("nearest_values")) rm(nearest_values)
  cleanup_memory()
})
...
system.time({
  ...
  # Clean up temporary objects (keep vel, ang, mn for trajectory calculations)
  rm(x1, x2, vt, vg, gv, r, dv)
  cleanup_memory()
})
...
system.time({
  ...
  # Clean up before expensive trajectory calculation
  cleanup_memory()
})
