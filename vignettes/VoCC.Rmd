...
marshift$GV <- terra::extract(abs(gv), terra::buffer(coords, buffer_size), 
...
# system.time({
#   traj <- map(lonlat %>% group_split(lineID), 
#               ~ voccTraj(lonlat = .x, vel, ang, mn, tyr = 50, tstep = 1/12))
# })
...
eez_simp <- sf::st_simplify(EEZs, preserveTopology = TRUE, dTolerance = 0.5)
...
ggplot(data = lns) + geom_sf(aes(colour = cellID))
