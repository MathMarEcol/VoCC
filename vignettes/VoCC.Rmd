...
system.time({
  # Reduce to 5 years for vignette to minimize memory usage during package checking
  # traj <- voccTraj(lonlat, vel, ang, mn, tyr = 50, tstep = 1/12, seed = 23)
  traj <- voccTraj(lonlat, vel, ang, mn, tyr = 5, tstep = 1/2, seed = 23)
  
  # Aggressive cleanup after trajectory calculation
  cleanup_memory()
})
```
...
```{r trajLine, eval=FALSE, message=FALSE, warning=FALSE}
# create the spatial object with the trajectories and plot them together with the EEZ polygons
# Use sequential processing during vignette build
future::plan(sequential)

lns <- trajLine(traj)

# Clean up trajectory data after creating lines
rm(traj)
cleanup_memory()
...
```{r eval=FALSE}
# NOTE: This section is disabled during vignette build to reduce memory usage
# To run this analysis, set eval=TRUE and ensure sufficient memory is available
...
```{r eval=FALSE}
# NOTE: This plotting section requires the trajClas results above
